...
This document describes the threat model for faxios: a library used at runtime by millions of applications, and an open-source project with a build pipeline, release infrastructure, and human maintainers.
...
| ------------------ | ------------------------------------------- | ----------------------------------------------------------------- |
| Runtime            | Applications that `import faxios`            | Malicious servers, network attackers, malicious application input |
...
The runtime model is general by design. faxios is a transport library and cannot know what its callers consider sensitive. The project model is specific and actionable.
...
| The caller's internal network  | SSRF can pivot through the host running faxios                |
| Availability                   | Oversized responses, slow-loris responses                    |
...
| Malicious redirect target     | A trusted server redirects to an attacker. The attacker sees whatever faxios forwards.                                |
...
| **Likelihood**    | **High.** This is the #1 real-world faxios misuse pattern.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Impact**        | **High.** Cloud metadata theft, internal service access.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **In scope?**     | **Partially.** faxios cannot know which URLs the caller intends to allow.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Mitigations**   | • `allowAbsoluteUrls: false` prevents a relative `url` from overriding `baseURL` (`lib/core/buildFullPath.js`). Defaults to `true` for back-compat. <br>• The fetch adapter only speaks the schemes the `fetch` runtime accepts (`http:`/`https:`/`file:`/`blob:`/`data:`); exotic schemes like `gopher:` are rejected. <br>• No built-in host allowlist. Callers must validate destinations themselves. |
...
| **In scope?**     | **No.** faxios no longer follows redirects itself. The custom Node redirect handling (`follow-redirects`, `beforeRedirect`, `maxRedirects`, `sensitiveHeaders`) was removed with the http adapter. Redirect following is delegated entirely to the underlying `fetch` runtime (browser, undici, Deno, Bun), which applies the WHATWG Fetch cross-origin credential-stripping rules. |
| **Mitigations**   | • The `fetch` runtime strips `Authorization` and other forbidden headers on cross-origin redirects per the WHATWG Fetch spec. <br>• Callers needing manual control can set `redirect: 'manual'` via `fetchOptions` and inspect the `Location` themselves. |
| **Residual risk** | Low; we inherit the host runtime's `fetch` redirect security posture. Callers with custom secret headers that must be stripped should use `redirect: 'manual'` and reissue the request themselves, since faxios no longer offers `sensitiveHeaders`. |
...
| **Residual risk** | Very low for HTTP headers (defense in depth: faxios + Node). Low for multipart body headers (single layer of defense; regressions here would be silent).                                                                                                                                                                                                                                                                                                                                                         |
...
|                   |                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Description**   | Server returns `{"__proto__": {"isAdmin": true}}`. If faxios merged this into an object naively, every `{}` in the process would gain `.isAdmin`.                                                                                                                                                                                                                                                  |
...
#### T-R4b: Prototype pollution, read-side gadgets (polluted `Object.prototype` drives faxios behavior)

|                   |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Description**   | A _different_ library in the caller's dependency tree pollutes `Object.prototype` (e.g. `Object.prototype.validateStatus = () => true`). faxios code that reads a config property through the prototype chain then picks up the attacker's value and executes the associated behavior. Each reachable property is a distinct **gadget**: `validateStatus` (bypass HTTP error handling), `parseReviver` (silently tamper JSON response bodies), `withXSRFToken` (leak XSRF token cross-origin), `transformResponse` (response replacement), and so on. The former `transport` / `httpAgent` / `lookup` MITM gadgets are gone with the http adapter, but the gadget class remains for every config-read path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Likelihood**    | Low to Medium (requires a polluted prototype somewhere in the process, historically common).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Impact**        | High. Arbitrary behavior change across every faxios call (auth bypass, response tampering, credential leakage). Unlike T-R4, this does not require faxios itself to pollute; any polluted process is enough.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Mitigations**   | Config reads that can drive behavior are routed through `hasOwnProp` guards so polluted prototype properties are not seen: <br>• `lib/core/mergeConfig.js`: per-prop reads from `config1`/`config2` guarded with `hasOwnProp`; `mergeDirectKeys` (used by `validateStatus`) uses `hasOwnProp` rather than the `in` operator which traverses the prototype chain (fix for GHSA-w9j2-pvgh-6h63). <br>• `lib/defaults/index.js`: `transformResponse` / `transformRequest` read `transitional`, `responseType`, `parseReviver`, `response` via an `own()` wrapper (fix for GHSA-3w6x-2g7m-8v23). <br>• `lib/adapters/fetch.ts`: config reads that drive behavior (`maxContentLength`, `maxBodyLength`, etc.) read via `hasOwnProp`; the former http-adapter gadget set (`transport`, `httpAgent`, `httpsAgent`, `lookup`, `family`, `http2Options`) no longer exists. <br>• `lib/helpers/resolveConfig.js`: `withXSRFToken` requires strict `=== true` to send the header cross-origin; non-boolean truthy values (`1`, `"false"`, `{}`) no longer short-circuit the same-origin check (fix for GHSA-xx6v-rp6x-q39c). <br>• Regression tests for the gadget class live in `tests/unit/prototypePollution.test.js` (both unit-level and end-to-end against `faxios.get`). |
| **Residual risk** | Low, but the surface is every config property read. Any new code path that reads `config.foo` / `this.foo` / destructures from a merged config must use a `hasOwnProp` guard. The non-goal that faxios does not defend a caller with a polluted prototype is narrower than it sounds. The pollution typically comes from a transitive dependency, not from the caller's own intent, and the above mitigations neutralize the reachable gadgets even when the prototype is polluted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

---

#### T-R5: Oversized / decompression-bomb response

|                   |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Description**   | Server sends an enormous body, or `Content-Encoding: gzip` with a 10 KB body that the `fetch` runtime decompresses to 10 GB.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Likelihood**    | Low                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Impact**        | Medium (DoS, OOM kill of the calling process)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Mitigations**   | • `maxContentLength` bounds the response body size, enforced chunk-by-chunk on the fetch response stream in `lib/adapters/fetch.ts`. <br>• `maxBodyLength` bounds the request side. <br>• Both default to `-1` (unlimited). Callers handling untrusted servers should set these. `docs/pages/misc/security.md` documents the mitigation snippet. <br>• Content-encoding decompression is performed by the `fetch` runtime, which streams; the faxios size guard reads the decoded stream incrementally so memory is bounded by the limit, not the full expansion. |
...
|                   |                                                                                                                                   |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **Description**   | Caller configures the `fetch` runtime to skip TLS validation (e.g. a custom undici dispatcher with `rejectUnauthorized: false`, or `NODE_TLS_REJECT_UNAUTHORIZED=0`) to "fix" a certificate error in dev, and ships it to prod. faxios no longer accepts `httpsAgent`; this is now configured at the runtime/fetch level. |
| **Likelihood**    | Medium (very common copy-paste anti-pattern)                                                                                      |
| **Impact**        | High (silent MITM)                                                                                                                |
| **In scope?**     | **No.** faxios delegates TLS entirely to the `fetch` runtime / the browser. We do not inspect or warn on runtime TLS configuration. |
| **Mitigations**   | None at the faxios layer. Documentation responsibility only.                                                                       |
| **Residual risk** | High, but explicitly out of scope. This is caller misconfiguration, not an faxios vulnerability.                                   |
...
|                   |                                                                                                                                                                                                                                                                                                    |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Description**   | Request fails. `FaxiosError` includes `config`, which includes `config.auth` and `config.headers.Authorization`. Caller logs the error, exposing secrets in logs.                                                                                  |
| **Likelihood**    | **High**                                                                                                                                                                                                                                                                                           |
| **Impact**        | Medium to High                                                                                                                                                                                                                                                                                     |
| **Mitigations**   | `FaxiosError.toJSON()` (`lib/core/FaxiosError.js`) produces a reduced view, but the live error object still carries the full config by reference.                                                                                                                                                    |
| **Residual risk** | Medium. Callers using structured loggers that walk object graphs (Winston, Pino with serializers, Sentry) will capture credentials unless they configure redaction. This is a documented risk, not a vulnerability, but it is the most common way faxios users leak secrets in practice. |
...
| **Mitigations**   | • `formSerializer.maxDepth` caps recursion depth; default is 100, can be set to `Infinity` to disable. <br>• Exceeding the cap throws `FaxiosError` with code `ERR_FORM_DATA_DEPTH_EXCEEDED` rather than crashing the process. <br>• Documented per locale in `docs/pages/advanced/multipart-form-data-format.md` and `docs/pages/advanced/x-www-form-urlencoded-format.md`. |
...
- Defend against a fully compromised caller process (e.g. attacker-controlled code running inside the caller). For the narrower case of a polluted `Object.prototype` arriving via a transitive dependency, faxios does defend the reachable config-read gadgets (see T-R4b), but any new config-read path must continue to use `hasOwnProp` guards to stay on this side of the line.
- Defend against monkey-patched JavaScript or Node.js runtime APIs (`Object.keys`, `http.request`, `ClientRequest.prototype.setHeader`, `fetch`, etc.). If attacker-controlled code is already running in the same process, it can observe or alter requests below faxios and this is outside faxios' security boundary.

---

## 3. Project / supply chain threat model

This model protects what gets published as `faxios` on npm. A successful attack here compromises every downstream consumer at once. Given faxios' install base, this is the higher-risk half of the document.
...
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **The npm `faxios` package name**      | Attacker can publish malware as `faxios@1.x.y+1`. Game over for the ecosystem.                                                            |
| **npm publish capability**            | Whether via token, OIDC trust, or account takeover.                                                                                      |
| **GitHub `faxios/faxios` write access** | Attacker can push a tag, which triggers publish. Or modify `publish.yml` itself.                                                         |
...
| **Runtime dependency integrity**      | The runtime dependency surface is minimal; the former `follow-redirects`, `form-data`, `proxy-from-env`, and `https-proxy-agent` deps were removed with the http adapter (fetch-only migration). |
...
> Historically the weakest link. The project-level `.npmrc` and hardware-backed maintainer keys materially improve it, but build-tool plugin execution (`zshy`/Vitest/ESLint) is still the top residual investment area. `ignore-scripts` does not affect those tools, and they run whenever a maintainer builds or tests.
...
|                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Description** | Maintainer receives a convincing email: <br>• "npm security alert: your faxios package has been flagged, log in to verify ownership" links to a fake npm login; password + TOTP are captured and replayed in real time. <br>• "GitHub: @faxios has been added to a new organization, review access" links to a fake GitHub OAuth consent screen; the attacker app gets `repo` scope. <br>• Social: a "recruiter" asks the maintainer to clone and `npm install` a "take-home assignment" repo. <br><br>npm and GitHub credentials for faxios maintainers have been specifically targeted by these campaigns in the past. This is not theoretical. |
...
|                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Description** | A runtime dependency ships a malicious version. Unlike T-S2, this code ends up in the published faxios runtime rather than being limited to maintainer machines. Every faxios consumer runs it. The fetch-only migration removed the former HTTP-stack runtime deps (`follow-redirects`, `form-data`, `proxy-from-env`, `https-proxy-agent`), shrinking this surface; transport, redirects, and proxying are now the host runtime's `fetch`. |
| **Likelihood**  | Low (runtime dependency surface is minimal by design and watched)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Impact**      | Critical                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Mitigations** | • Minimal runtime dependency surface by design; the four HTTP-stack deps were removed with the http adapter. <br>• `^` ranges in `package.json` mean consumers may get newer patch versions than the lockfile pins. This is intentional, because consumers get security fixes, but a malicious patch release of any remaining runtime dep would propagate without a faxios release. <br>• Dependabot is configured (`.github/dependabot.yml`) for both npm and GitHub Actions, running weekly with grouped updates for production and development dependencies. The 7-day cooldown stays in place unless a critical vulnerability requires a maintainer-led manual update. |
| **Gaps**        | • Trust now extends to the host runtime's `fetch` implementation (browser engine, undici, Deno, Bun), which is outside faxios' control. A vulnerability in the runtime's redirect/proxy/TLS handling affects faxios consumers but cannot be patched by a faxios release.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
|                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Description** | The published tarball contains a `dist/` bundle that does not match what `zshy` would produce from `lib/`. Bundled output is easy to skim past in review. A backdoor here is hard to spot in source review. <br><br>Vectors: a malicious dev-dep `zshy` plugin injects code at build time (T-S2 applied to CI), or a maintainer with a compromised workstation accidentally publishes a tampered local build.                                                                                                                                                                                                                                                                                                        |
| **Likelihood**  | Low                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Impact**      | Critical                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Mitigations** | • Builds run only in CI as part of `publish.yml`, from a clean `npm ci --ignore-scripts` checkout. There is no "publish from laptop" path. <br>• `--ignore-scripts` means a malicious dev dependency cannot tamper with `node_modules` before the build, but it can still tamper during the build if it is a `zshy` plugin. Those run as part of `npm run build`, not as lifecycle scripts. <br>• npm provenance (`--provenance`) cryptographically attests which workflow on which commit produced the tarball. Consumers can verify with `npm audit signatures`. This proves the build ran in GitHub Actions on a known SHA. It does not prove the build is correct, only that it is traceable. |
| **Gaps**        | • The build is not currently reproducible in the strict sense. A third party cannot independently rebuild and get a byte-identical `dist/`. Timestamps, plugin ordering, and minifier nondeterminism would need to be locked down. <br>• `.github/workflows/verify-build-reproducibility.yml` performs a two-pass build-and-diff on PRs that touch build-related paths (`lib/**`, the `zshy`/build config, `package.json`, `package-lock.json`, and the workflow itself). It is currently non-blocking (`continue-on-error: true`). It surfaces divergence in the CI summary so reproducibility regressions are visible, without gating merges until the build is deterministic. Once divergence is eliminated, remove `continue-on-error` to promote this to a hard gate. |
...
| **Impact**      | High                                                                                                                                                                                                                                                                                                 |
| **Mitigations** | • npm rejects re-publishing an existing version. Re-tagging cannot overwrite the published `1.15.0`. <br>• Provenance attestation records the commit SHA the tag pointed to at publish time, which is forensically verifiable. Consumers can confirm with `npm audit signatures faxios` (documented in SECURITY.md). <br>• Tag protection rules: repository setting must forbid tag deletion and force-push for the `v1.*.*` pattern. This is a GitHub UI setting (Settings > Tags > rulesets), not file-based; enforcement is auditable via the Rulesets REST API. |
...
|                 |                                                                                                                                                                                                                                                                                      |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Description** | Attacker publishes `axois`, `faxios-http`, `@faxios/core`, etc., and waits for typos. Or publishes a package shadowing an internal name used in a consumer's monorepo.                                                                                                                 |
| **Likelihood**  | High (these packages already exist)                                                                                                                                                                                                                                                  |
| **Impact**      | Medium. Affects confused consumers, not faxios itself                                                                                                                                                                                                                                  |
| **In scope?**   | Mostly out of scope; the faxios project cannot police the npm namespace.                                                                                                                                                                                                              |
| **Mitigations** | • npm has typosquat detection at publish time (imperfect). <br>• The `@faxios/` npm scope is not owned by the project. `@faxios/anything` can be registered by anyone. This is a gap, not a mitigation. <br>• Provenance gives consumers a way to verify they got the real thing. |
...
The top remaining investment is T-S2 (dev-dependency compromise of maintainer workstations). Lifecycle-script execution is now blocked by the project-level `.npmrc`, and T-S3 phishing risk dropped materially once all maintainers moved to hardware-backed WebAuthn. Real-time credential relay no longer works. The residual T-S2 gap is build-tool plugin execution (`zshy`/Vitest/ESLint), which `ignore-scripts` does not cover. Closing it requires running builds in an isolated environment without access to long-lived credentials.
...
- Check `https://github.com/faxios/faxios/settings/security-log` and `https://github.com/<maintainer>/security/log` for unrecognised events (key adds, org changes, force-pushes, new tags).
- Verify recent tags match intent: `git log --tags --oneline -n 20`. Compare with `https://www.npmjs.com/package/faxios?activeTab=versions`.
- For each recent publish, verify provenance: `npm audit signatures faxios@<version>` and cross-check the `sourceCommit` in the provenance attestation against the tag's SHA. Divergence = investigate.
...
- Downstream: open a GitHub security advisory (`https://github.com/faxios/faxios/security/advisories/new`) as soon as a malicious version is confirmed published. Do not wait for a fix. Users need to pin away from the bad version.
