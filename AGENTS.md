...
faxios is a promise-based HTTP client for the browser and Node.js. The default instance is exported from `lib/faxios.js` via `index.js`. Browser builds use the XHR or Fetch adapter; Node uses the HTTP/HTTPS adapter. Platform selection lives in `lib/platform/`.
...
- Runtime package exports are split by environment: browser/react-native map Node HTTP/platform files to browser/null replacements, while Node CJS ships as `dist/node/faxios.cjs`.
- Keep public runtime exports, `index.d.ts` (ESM types), and `index.d.cts` (CJS `export = faxios` types) in sync for API changes.
