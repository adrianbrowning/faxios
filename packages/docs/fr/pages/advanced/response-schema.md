...
Chaque requête faxios se résout vers un objet de réponse ayant la structure suivante. Le schéma est cohérent aussi bien dans les environnements navigateur que Node.js.
...
```js
{
  ...
  // La configuration faxios utilisée pour cette requête, incluant baseURL,
...
faxios résout la promise pour toute réponse 2xx et rejette pour tout ce qui est en dehors de cette plage par défaut. Vous pouvez personnaliser ce comportement avec l'option de configuration `validateStatus` :
