...
faxios 可能会抛出多种不同类型的错误，有些来自 faxios 本身，有些来自服务器或客户端。下表列出了所抛出错误的基本结构：

| 属性    | 说明                                                                                                                                          |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| message | 错误信息的简要摘要，以及失败时的状态码。                                                                                                      |
| name    | 定义错误的来源，对于 faxios 来说始终是 `FaxiosError`。                                                                                          |
| stack   | 提供错误的堆栈跟踪。                                                                                                                          |
| config  | 包含用户在发起请求时定义的特定实例配置的 faxios 配置对象。                                                                                     |
| code    | 表示 faxios 内部识别的错误，下表列出了 faxios 内部错误的具体说明。                                                                              |
| status  | HTTP 响应状态码。常见 HTTP 响应状态码的含义请参阅[此处](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes)。                            |

以下是 faxios 内部可能出现的错误列表：

| 错误代码                  | 说明                                                                                    |
| ------------------------- | --------------------------------------------------------------------------------------- |
| ERR_BAD_OPTION_VALUE      | faxios 配置中提供了无效或不支持的值。                                                    |
...
| ERR_NOT_SUPPORT           | 当前 faxios 环境不支持该功能或方法。                                                     |
| ERR_INVALID_URL           | faxios 请求提供了无效的 URL。                                                            |
...
faxios 的默认行为是在请求失败时 reject Promise。不过，你也可以捕获错误并按需处理。以下是捕获错误的示例：
...
为避免从 `error.config` 中泄露敏感信息，可在请求配置中传入 `redact` 数组。当调用 `FaxiosError#toJSON()` 时，匹配的配置键会在任意深度上以不区分大小写的方式被脱敏。
...
当请求超过其配置的 `timeout` 时，faxios 默认会以 `ECONNABORTED` 拒绝。设置 `transitional.clarifyTimeoutError: true` 可改为收到 `ETIMEDOUT`，便于将超时错误与其他中止情况区分开。
...
async function fetchWithTimeout() {
  try {
    ...
    if (faxios.isAxiosError(error)) {
      ...
      console.error("faxios 错误：", error.message);
      return;
    }
