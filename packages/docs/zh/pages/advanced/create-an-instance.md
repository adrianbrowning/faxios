...
`faxios.create()` 允许你创建一个预配置的 faxios 实例。该实例与默认 `faxios` 对象拥有相同的请求和响应 API，但会将你提供的配置作为每次请求的基础配置。对于任何超过单文件规模的应用，这是使用 faxios 的推荐方式。
...
`create` 方法接受完整的[请求配置](/pages/advanced/request-config)对象。之后可以像使用默认 faxios 对象一样使用该实例：
