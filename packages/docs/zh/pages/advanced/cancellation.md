...
从 v0.22.0 起，faxios 支持使用 AbortController 以简洁的方式取消请求。该功能在浏览器和 Node.js（使用支持 AbortController 的 faxios 版本）中均可使用。要取消请求，需要创建一个 `AbortController` 实例，并将其 `signal` 传入请求的 `signal` 选项。
...
你可以使用同一个取消令牌或 AbortController 取消多个请求。如果在 faxios 请求开始时取消令牌已处于已取消状态，则请求会立即被取消，不会尝试发起实际的网络请求。
