...
使用 `faxios.isAxiosError` 类型守卫可以在 `catch` 块中安全地收窄 `unknown` 错误。收窄之后，你便可以在完整的类型支持下访问 `error.response`、`error.config` 和 `error.code` 等 faxios 专有属性。
...
使用 `faxios.isCancel<T>()` 可以将取消错误收窄为 `CanceledError<T>`：
...
将 `faxios.create` 的结果标注为 `AxiosInstance`，并将请求拦截器标注为 `InternalAxiosRequestConfig`，即可对自定义客户端实现端到端的类型检查：
...
faxios 的请求方法对响应数据类型是泛型的。向 `faxios.get<T>`（以及其他别名）传入类型参数即可为 `response.data` 添加类型：
