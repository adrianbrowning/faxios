...
describe("helpers::progressEventReducer", () => {
  ...
  it("should ignore a NaN loaded value instead of poisoning later progress", () => {
    const events: Array<FaxiosProgressEvent> = [];
    const [ onProgress, flush ] = progressEventReducer(
      data => {
        events.push(data);
    ...
    onProgress({ lengthComputable: true, loaded: Number.NaN, total: 100 });
    flush();

    expect(events.length).toBe(0);

    onProgress({ lengthComputable: true, loaded: 40, total: 100 });
    flush();
