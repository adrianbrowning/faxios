...
// @ts-expect-error no declaration file for dev-null
...
// @ts-expect-error no declaration file for express
import express from "express";
// @ts-expect-error no declaration file for multer
...
describe("supports http with nodejs", () => {
  ...
  function toleranceRange(positive: any, negative: any) {
    const p = 1 + positive / 100;
    const n = 1 - negative / 100;

    return (actualValue: any, value: any) =>
      actualValue > value ? actualValue <= value * p : actualValue >= value * n;
  }
  ...
  class HangingConnectSocket extends stream.Duplex {
    ...
    override _write(_chunk: any, _encoding: any, callback: any) {
      callback();
    }
  ...
  it("should support IPv4 literal strings", async () => {
    ...
    try {
      const { data: responseData } = await (axios as unknown as TypedFaxiosStatic).get(
  ...
  it("should throw an error if the timeout property is not parsable as a number", async () => {
    ...
    try {
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).get(`http://localhost:${(server.address() as AddressInfo).port}`, {
  ...
  it("should respect the timeout property during TCP connect with maxRedirects set to 0", async () => {
    ...
    let guardTimer;
    const request = (axios as unknown as TypedFaxiosStatic).get("http://connect-timeout.test/", {
      httpAgent: agent,
      maxRedirects: 0,
      proxy: false,
  ...
  it("should not time out immediately for timeout set to zero during TCP connect", async () => {
    const controller = new AbortController();
    const agent = new HangingConnectAgent();
    const request = (axios as unknown as TypedFaxiosStatic)
  ...
  it("should redirect", async () => {
    ...
    try {
      const response = await (axios as unknown as TypedFaxiosStatic).get(
  ...
  it("should support max redirects", async () => {
    ...
    try {
      await (axios as unknown as TypedFaxiosStatic).get(`http://localhost:${(server.address() as AddressInfo).port}`, {
        maxRedirects: 3,
      });
    } catch (error: any) {
    ...
    const proxy = await startHTTPServer(
      (req, res) => {
        ...
        const opts = {
          ...
          port: Number(targetUrl.port) || undefined,
          path: targetUrl.pathname,
          method: req.method,
        };

        const request = http.get(opts as any, (response) => {
  ...
  it("should strip sensitiveHeaders on cross-origin redirect", async () => {
    ...
    try {
      await (axios as unknown as TypedFaxiosStatic).get(`http://localhost:${(origin.address() as AddressInfo).port}/src`, {
        maxRedirects: 5,
  ...
  it("should preserve sensitiveHeaders on same-origin redirect", async () => {
    ...
    const server = await startHTTPServer((req: any, res: any) => {
      requestCount++;
      if (requestCount === 1) {
    ...
    try {
      await (axios as unknown as TypedFaxiosStatic).get(`http://localhost:${(server.address() as AddressInfo).port}/src`, {
        maxRedirects: 5,
  ...
  it("should strip sensitiveHeaders configured on an instance", async () => {
    ...
    try {
      await (client as unknown as TypedFaxiosInstance).get(`http://localhost:${(origin.address() as AddressInfo).port}/src`, {
        maxRedirects: 5,
      });
  ...
  it("should reject invalid sensitiveHeaders config", async () => {
    await assert.rejects(
      (axios as unknown as TypedFaxiosStatic).get("http://localhost:1/", { sensitiveHeaders: "X-API-Key" as any }),
      (error: any) => {
        assert.strictEqual(error.code, FaxiosError.ERR_BAD_OPTION_VALUE);
        assert.strictEqual(
          error.message,
    ...
    await assert.rejects(
      (axios as unknown as TypedFaxiosStatic).get("http://localhost:1/", { sensitiveHeaders: [null as any] }),
      (error: any) => {
        assert.strictEqual(error.code, FaxiosError.ERR_BAD_OPTION_VALUE);
        assert.strictEqual(
          error.message,
  ...
  it("should fail closed when sensitiveHeaders redirect origin cannot be parsed", () => {
    ...
    assert.strictEqual(
      __isSameOriginRedirect({ href: "http://localhost/final" } as any, undefined as any),
      false,
    );
  });
  ...
  it("should wrap interceptor errors and keep stack", async () => {
    ...
    (axiosInstance as unknown as TypedFaxiosInstance).interceptors.request.use((_res: any) => {
  ...
  it("should preserve the HTTP verb on redirect", async () => {
    ...
    try {
      const response = await (axios as unknown as TypedFaxiosStatic).head(
        `http://localhost:${(server.address() as AddressInfo).port}/one`,
      );
  ...
  describe("compression", async () => {
    ...
    it("should support transparent gunzip", async () => {
      ...
      const zipped: any = await new Promise((resolve, reject) => {
      ...
      try {
        await assert.rejects(
          ...
          (error: any) => {
            ...
            assert.strictEqual(
              (error.response.headers as any).get("x-stream-error"),
              "yes",
            );
    ...
    describe("algorithms", () => {
      ...
      const deflateRaw = async (value: any) =>
        new Promise((resolve, reject) => {
      ...
      const brotliCompress = async (value: any) =>
        new Promise((resolve, reject) => {
      ...
      const zstdCompress = async (value: any) =>
        new Promise((resolve, reject) => {
  ...
  it("should ignore inherited proxy when http adapter receives a plain config", async () => {
    ...
    try {
      ...
      const response = await (httpAdapter as any)({
        method: "get",
        url: `http://127.0.0.1:${(target.address() as AddressInfo).port}/direct`,
        headers: {},
        maxRedirects: 0,
        maxContentLength: -1,
        maxBodyLength: -1,
      ...
      await stopHTTPServer(target as any);
      await stopHTTPServer(proxy);
    }
  });
  ...
  it("should ignore inherited paramsSerializer when http adapter receives a plain config", async () => {
    ...
    try {
      server = await startHTTPServer((req: any, res: any) => {
        res.setHeader("Content-Type", "application/json");
      ...
      const response = await (httpAdapter as any)({
        method: "get",
        url: `http://127.0.0.1:${(server.address() as AddressInfo).port}/direct`,
        headers: {},
        params: { value: "a b" },
        proxy: false,
        maxRedirects: 0,
  ...
  it("should preserve basic auth across same-origin 303 POST -> GET redirect", async () => {
    ...
    try {
      const auth = { username: "foo", password: "bar" };
      const response = await (axios as unknown as TypedFaxiosStatic).post(
  ...
  it("should allow the Content-Length header to be overridden", async () => {
    ...
    try {
      ...
      await (axios as unknown as TypedFaxiosStatic).post(`http://localhost:${(server.address() as AddressInfo).port}/`, "foo", {
        headers,
      });
    } finally {
      await stopHTTPServer(server);
  ...
  it("should support max content length for redirected", async () => {
    ...
    try {
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).get(`http://localhost:${(server.address() as AddressInfo).port}/one`, {
          maxContentLength: 2000,
        }),
        (error: any) => {
          assert.strictEqual(
  ...
  it("should support max body length", async () => {
    ...
    try {
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).post(
          `http://localhost:${(server.address() as AddressInfo).port}/`,
          {
            data,
          },
  ...
  it("should enforce maxContentLength for streamed responses", async () => {
    ...
    try {
      ...
      assert.strictEqual((err as any).message, "maxContentLength size of 1024 exceeded");
      assert.ok(
        bytesRead <= 1024 * 64,
  ...
  it("should enforce maxBodyLength for streamed uploads with maxRedirects: 0", async () => {
    ...
    try {
      ...
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).post(`http://localhost:${(server.address() as AddressInfo).port}/`, source, {
          maxBodyLength: 1024,
          maxRedirects: 0,
  ...
  it("should support sockets", async () => {
    ...
    try {
      const response = await (axios as unknown as TypedFaxiosStatic)({
  ...
  describe("streams", () => {
    ...
    it("should destroy the response stream with an error on request stream destroying", async () => {
      ...
      const { data } = await (axios as unknown as TypedFaxiosStatic).post(
        `http://localhost:${(server.address() as AddressInfo).port}/`,
        requestStream,
        {
          responseType: "stream",
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          stream.pipeline(data, devNull(), (error: any) => {
            if (error) {
              reject(error);
              return;
            }
  ...
  it("should support HTTPS proxies", async () => {
    ...
    // Note: closeServer helper removed (unused)

    const server: any = await new Promise<any>((resolve, reject) => {
      const httpsServer = https.createServer(tlsOptions, (_req: any, res: any) => {
        res.setHeader("Content-Type", "text/html; charset=UTF-8");
        res.end("12345");
      });
    ...
    const tunnelingAgent = new (HttpsProxyAgent as any)({
      protocol: "https:",
      host: "127.0.0.1",
      port: (proxy.address() as AddressInfo).port,
    ...
    try {
      ...
      assert.ok(
        connectTargets[0]?.startsWith(`localhost:${(server.address() as AddressInfo).port}`),
  ...
  it("should CONNECT-tunnel HTTPS targets through an HTTP proxy by default (issue #6320)", async () => {
    ...
    const origin: any = await new Promise<any>((resolve, reject) => {
    ...
    const captured: { plaintext: number; connectTargets: string[]; connectAuth: (string|null)[]; plaintextUrl?: string } = { plaintext: 0, connectTargets: [], connectAuth: [] };
    const upstreamSockets: any[] = [];
    const proxy: any = await new Promise<any>((resolve, reject) => {
      ...
      p.on("connect", (req, clientSocket, head) => {
        ...
        captured.connectAuth.push((req.headers["proxy-authorization"] as string) || null);
  ...
  it("should apply httpsAgent TLS options to CONNECT-tunneled origins (issue #10953)", async () => {
    ...
    const origin: any = await new Promise<any>((resolve, reject) => {
      const s = https.createServer(tlsOptions, (_req: any, res: any) => {
        res.setHeader("Content-Type", "text/html; charset=UTF-8");
    ...
    const captured: { plaintext: number; connectTargets: string[] } = { plaintext: 0, connectTargets: [] };
    const upstreamSockets: any[] = [];
    const proxy: any = await new Promise<any>((resolve, reject) => {
  ...
  it("should surface a CONNECT 407 from the proxy as an FaxiosError (issue #6320)", async () => {
    const proxy: any = await new Promise<any>((resolve, reject) => {
      const p = http.createServer();
    ...
    try {
      await assert.rejects(
        async () => {
          await (axios as unknown as TypedFaxiosStatic).get("https://127.0.0.1:1/", {
            proxy: {
              host: "127.0.0.1",
              port: (proxy.address() as AddressInfo).port,
              protocol: "http",
  ...
  it("should not use proxy for localhost with trailing dot when listed in no_proxy", async () => {
    ...
    try {
      await assert.rejects((axios as unknown as TypedFaxiosStatic).get("http://localhost.:1/", { timeout: 100 }));
      assert.equal(
        proxyRequests,
        0,
  ...
  it("should not use proxy for bracketed IPv6 loopback when listed in no_proxy", async () => {
    ...
    try {
      await assert.rejects((axios as unknown as TypedFaxiosStatic).get("http://[::1]:1/", { timeout: 100 }));
  ...
  describe("when invalid proxy options are provided", () => {
    it("should throw error", async () => {
      ...
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).get("https://test-domain.abc", { proxy }),
        (error: any) => {
  ...
  describe("different options for direct proxy configuration (without env variables)", () => {
    ...
    for (const test of testCases) {
      it(test.description, () => {
        const options: { headers: Record<string, any>; beforeRedirects: Record<string, any>; [key: string]: any } = { headers: {}, beforeRedirects: {} };
        __setProxy(options, test.proxyConfig, destination, undefined as any, undefined);
  ...
  describe("Host header preservation when forwarding through a proxy (#10805)", () => {
    ...
    it("defaults the Host header to the request target when the user does not set one", () => {
      const options: { headers: Record<string, any>; beforeRedirects: Record<string, any>; [key: string]: any } = {
        headers: {},
        beforeRedirects: {},
        hostname: "127.0.0.1",
        port: 4000,
      ...
      __setProxy(options, proxyConfig, "http://127.0.0.1:4000/", undefined as any, undefined);
    ...
    it("preserves a user-supplied Host header regardless of casing", () => {
      ...
      assert.strictEqual((options.headers as any).Host, "example.com");
      assert.strictEqual((options.headers as any).host, undefined);
    });
    ...
    it("preserves a user-supplied Host header across a redirect re-invocation", () => {
      ...
      __setProxy(options, proxyConfig, "http://127.0.0.1:4000/", true, undefined);
    ...
    it("ignores polluted prototype Host fields when detecting user-supplied headers", () => {
      (Object.prototype as any).host = "polluted.example.com";
  ...
  describe("Proxy-Authorization header leak on redirect", () => {
    it("clears a stale Proxy-Authorization header when redirected request resolves to no proxy (configProxy=false)", () => {
      ...
      const redirectOptions: { headers: Record<string, any>; beforeRedirects: Record<string, any>; [key: string]: any } = {
    ...
    it("clears a stale Proxy-Authorization header when environment-derived proxy is bypassed on redirect (NO_PROXY)", () => {
      ...
      try {
        ...
        __setProxy(options, undefined, "http://initial.example.com/start", undefined as any, undefined);
        assert.strictEqual(
          options.headers["Proxy-Authorization"],
          "Basic " + Buffer.from("user:pass", "utf8").toString("base64"),
    ...
    it("strips stale Proxy-Authorization when the beforeRedirects.proxy hook is invoked with configProxy=false", () => {
      ...
      __setProxy(options, false, "http://initial.example.com/start", undefined as any, undefined);
      assert.strictEqual(
      ...
      (options as any).beforeRedirects.proxy(redirectOptions);
    ...
    it("preserves a user-supplied Proxy-Authorization header on the initial request when no proxy is configured", () => {
      ...
      __setProxy(options, false, "http://example.com/start", undefined as any, undefined);
    ...
    it("does not forward Proxy-Authorization to a redirect target that resolves to no-proxy", async () => {
      ...
      const corpProxy = await startServer((req: any, res: any) => {
        proxySaw.push({
          url: req.url,
          proxyAuth: req.headers["proxy-authorization"],
        });
        res.writeHead(302, {
          Location: `http://127.0.0.1:${attackerPort}/final`,
        });
        res.end();
      });

      const attacker = await startServer((req: any, res: any) => {
        attackerSaw.push({
          url: req.url,
          proxyAuth: req.headers["proxy-authorization"],
          authorization: req.headers.authorization,
      ...
      const corpProxyPort = ((corpProxy as any).address() as AddressInfo).port;
      const originalHttpProxy = process.env.http_proxy;
      const originalNoProxy = process.env.no_proxy;
      ...
      try {
        await (axios as unknown as TypedFaxiosStatic).get("http://example.com/start");
  ...
  it("should combine baseURL and url", async () => {
    ...
    try {
      const response = await (axios as unknown as TypedFaxiosStatic).get("/foo", {
  ...
  describe("HTTPS CONNECT tunneling agent management", () => {
    const buildOptions = (): { headers: Record<string, any>; beforeRedirects: Record<string, any>; hostname: string; host: string; port: number; path: string; protocol: string; [key: string]: any } => ({
      headers: {},
      beforeRedirects: {},
      hostname: "example.com",
      host: "example.com",
    ...
    it("reuses the same tunneling agent for repeated requests through the same proxy", () => {
      ...
      __setProxy(a, proxyConfig, "https://example.com/", undefined as any, undefined);
      __setProxy(b, proxyConfig, "https://example.com/", undefined as any, undefined);
    ...
    it("respects a user-supplied HttpsProxyAgent without installing its own", () => {
      const userTunnel = new (HttpsProxyAgent as any)({
        protocol: "http:",
        hostname: "127.0.0.1",
        port: 9999,
      });
    ...
    it("does not strip a user-supplied HttpsProxyAgent on redirect", () => {
      ...
      __setProxy(redirectOptions, false, "https://redirect.example.com/", true, undefined);
      assert.strictEqual(
    ...
    it("strips its own tunneling agent on redirect when the redirect target has no proxy", () => {
      ...
      __setProxy(initial, proxyConfig, "https://example.com/", undefined as any, undefined);
      assert.ok(
      ...
      __setProxy(redirectOptions, false, "https://final.example.com/", true, undefined);
      assert.strictEqual(
        redirectOptions.agent,
        undefined,
  ...
  it("should return malformed URL", async () => {
    await assert.rejects((axios as unknown as TypedFaxiosStatic).get("tel:484-695-3408"), (error: any) => {
  ...
  it("should return unsupported protocol", async () => {
    await assert.rejects((axios as unknown as TypedFaxiosStatic).get("ftp:google.com"), (error: any) => {
  ...
  it("should throw an error if http server that aborts a chunked request", async () => {
    ...
    try {
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).get(`http://localhost:${(server.address() as AddressInfo).port}/aborted`, {
          timeout: 500,
        }),
        (error: any) => {
          ...
          assert.strictEqual(
            (error.response.headers as any).get("x-stream-aborted"),
            "yes",
          );
  ...
  it("should able to cancel multiple requests with CancelToken", async () => {
    ...
    try {
      const source = axios.CancelToken.source();
      const canceledStack: any[] = [];

      const requests = [1, 2, 3, 4, 5].map(async (id) => {
        try {
          await (axios as unknown as TypedFaxiosStatic).get("/foo/bar", {
            baseURL: `http://localhost:${(server.address() as AddressInfo).port}`,
            cancelToken: source.token,
          });
        } catch (error: any) {
  ...
  describe("FormData", () => {
    ...
    describe("prototype pollution", () => {
      ...
      it("should not use inherited Symbol.iterator for request or response headers", async () => {
        ...
        const stubTransport = {
          request(options: any, handleResponse: any) {
            ...
            req.destroy = () => {};
            req.end = () => {
              const res: any = new stream.Readable({ read() {} });
              res.statusCode = 200;
        ...
        try {
          ...
          const response = await (axios as unknown as TypedFaxiosStatic).get("http://stub.invalid/", {
            headers: {
          ...
          assert.strictEqual((capturedHeaders as any).Authorization || (capturedHeaders as any).authorization,
            "Bearer VALID_USER_TOKEN",
          );
          assert.strictEqual(
            (capturedHeaders as any)["X-Injected"] || (capturedHeaders as any)["x-injected"],
            undefined,
          );
          assert.strictEqual((response.headers as any).get("x-server"), "real");
          assert.strictEqual((response.headers as any).get("x-injected"), undefined);
        } finally {
      ...
      function pollute() {
        (Object.prototype as any)[toStringTagSym] = "FormData";
        (Object.prototype as any).append = () => {};
        (Object.prototype as any).getHeaders = () => ({
          "x-injected": "attacker",
          authorization: "Bearer ATTACKER_TOKEN",
        });
        (Object.prototype as any).pipe = function (d: any) {
          if (d && d.end) d.end();
          return d;
        };
        (Object.prototype as any).on = function () {
          return this;
        };
        (Object.prototype as any).once = function () {
          return this;
        };
      }

      function cleanup() {
        for (const k of pollutedKeys) delete (Object.prototype as any)[k];
        delete (Object.prototype as any)[toStringTagSym];
      }
      ...
      it("should not merge prototype-polluted getHeaders into outgoing request", async () => {
        ...
        try {
          pollute();
          await (axios as unknown as TypedFaxiosStatic).post(
            "http://stub.invalid/",
            { userId: 42 },
            {
    ...
    describe("formDataHeaderPolicy", () => {
      function createStubTransport(captureHeaders: any) {
        return {
          request(options: any, handleResponse: any) {
      ...
      it("preserves legacy getHeaders() propagation by default", async () => {
        ...
        await (axios as unknown as TypedFaxiosStatic).post("http://stub.invalid/", new CustomFormData(), {
        ...
        assert.ok(capturedHeaders, "transport was not invoked");
        const ct =
          (capturedHeaders as any)["Content-Type"] || (capturedHeaders as any)["content-type"];
        assert.match(ct, /multipart\/form-data/);
        assert.strictEqual((capturedHeaders as any)["x-injected"], "custom");
        assert.strictEqual((capturedHeaders as any)["x-forwarded-for"], "10.0.0.1");
        assert.strictEqual((capturedHeaders as any).Authorization || (capturedHeaders as any).authorization,
          "Bearer CUSTOM_TOKEN",
        );
        assert.strictEqual((capturedHeaders as any).Host || (capturedHeaders as any).host,
          "custom.example.com",
        );
      });
      ...
      it("only copies content headers when formDataHeaderPolicy is content-only", async () => {
        ...
        assert.match(ct, /multipart\/form-data/);
        assert.strictEqual((capturedHeaders as any)["x-injected"], undefined);
        assert.strictEqual((capturedHeaders as any)["x-forwarded-for"], undefined);
        assert.strictEqual((capturedHeaders as any).Authorization || (capturedHeaders as any).authorization,
          undefined,
        );
        assert.strictEqual((capturedHeaders as any).Host || (capturedHeaders as any).host,
  ...
  describe("toFormData helper", () => {
    it("should properly serialize nested objects for parsing with multer.js (express.js)", async () => {
      ...
      app.post("/", multer().none(), (req: any, res: any) => {
        res.send(JSON.stringify(req.body));
      });

      const server: any = await new Promise(
        (resolve, reject) => {
          const expressServer = (app as any).listen(0, () => resolve(expressServer));
      ...
      try {
        await Promise.all(
          [null, false, true].map((mode) =>
            (axios as unknown as TypedFaxiosStatic)
              .postForm(`http://localhost:${(server.address() as AddressInfo).port}/`, obj, {
    ...
    it("should only match explicit routes for express 5 form handlers", async () => {
      ...
      app.post("/unexpected", multer().none(), (_req: any, res: any) => {
      ...
      try {
        const rootResponse = await (axios as unknown as TypedFaxiosStatic).postForm(rootUrl, { foo: "bar" });
        ...
        await assert.rejects(
          () => (axios as unknown as TypedFaxiosStatic).postForm(`${rootUrl}/unexpected`, { foo: "bar" }),
          (error: any) => {
  ...
  describe("URLEncoded Form", () => {
    ...
    it("should parse nested urlencoded payloads and ignore mismatched content-type", async () => {
      ...
      try {
        const parsedResponse = await (axios as unknown as TypedFaxiosStatic).post(rootUrl, payload, {
          headers: {
            "content-type": "application/x-www-form-urlencoded",
          },
        });
        ...
        const ignoredResponse = await (axios as unknown as TypedFaxiosStatic).post(rootUrl, payload, {
          headers: {
            "content-type": "text/plain",
          },
        }) as any;
  ...
  describe("Data URL", () => {
    it("should support requesting data URL as a Buffer", async () => {
      ...
      const { data } = await (axios as unknown as TypedFaxiosStatic).get(dataURI);
    ...
    it("should support requesting data URL as a Blob (if supported by the environment)", async () => {
      ...
      const { data } = await (axios as unknown as TypedFaxiosStatic).get(dataURI, { responseType: "blob" }) as any;
    ...
    it("should support requesting data URL as a String (text)", async () => {
      ...
      const { data } = await (axios as unknown as TypedFaxiosStatic).get(dataURI, { responseType: "text" });
    ...
    it("should support requesting data URL as a Stream", async () => {
      ...
      const { data } = await (axios as unknown as TypedFaxiosStatic).get(dataURI, { responseType: "stream" }) as any;
      ...
      it("should support upload progress capturing", async () => {
        ...
        try {
          ...
          const { data } = await (axios as unknown as TypedFaxiosStatic).post(
            ...
            readable,
            {
              onUploadProgress: ({ loaded, total, progress, bytes, upload }: any) => {
                samples.push({
                  loaded,
                  total,
                  progress,
      ...
      it("should support download progress capturing", async () => {
        ...
        try {
          ...
          const { data } = await (axios as unknown as TypedFaxiosStatic).post(
            ...
            readable,
            {
              onDownloadProgress: ({ loaded, total, progress, bytes, download }: any) => {
                samples.push({
                  loaded,
                  total,
                  progress,
    ...
    it("should support upload rate limit", async () => {
      ...
      try {
        ...
        const { data } = await (axios as unknown as TypedFaxiosStatic).post(
          ...
          buf,
          {
            onUploadProgress: ({ loaded, total, progress, bytes, rate }: any) => {
              samples.push({
                loaded,
                total,
                progress,
    ...
    it("should support download rate limit", async () => {
      ...
      try {
        ...
        const { data } = await (axios as unknown as TypedFaxiosStatic).post(
          ...
          buf,
          {
            onDownloadProgress: ({ loaded, total, progress, bytes, rate }: any) => {
              samples.push({
                loaded,
                total,
                progress,
  ...
  it("should properly handle synchronous errors inside the adapter", async () => {
    await assert.rejects(
      () => (axios as unknown as TypedFaxiosStatic).get("http://192.168.0.285"),
      /Invalid URL/,
    );
  });
  ...
  describe("DNS", () => {
    ...
    it("should handle errors", async () => {
      await assert.rejects(async () => {
        await (axios as unknown as TypedFaxiosStatic).get("https://no-such-domain-987654.com", {
          lookup: lookup as any as any,
        });
      }, /ENOTFOUND/);
    });
  });
  ...
  describe("HTTP2", () => {
    const createHttp2Faxios = (baseURL: any) =>
      axios.create({
        baseURL,
        httpVersion: 2,
        http2Options: {
    ...
    it("should merge request http2Options with its instance config", async () => {
      ...
      const { data } = await (http2Faxios as unknown as TypedFaxiosInstance).get("/", {
        http2Options: {
          foo: "test",
        },
        adapter: (async (config: any) => ({ data: config.http2Options })) as any,
      });
    ...
    it("should support http2 transport", async () => {
      ...
      try {
        ...
        const { data } = await (http2Faxios as unknown as TypedFaxiosInstance).get(localServerURL);
    ...
    it("should support request payload", async () => {
      const server = await startHTTPServer(null as any, {
        useHTTP2: true,
        port: SERVER_PORT,
      });
      ...
      try {
        ...
        const http2Faxios = createHttp2Faxios(localServerURL);
        const payload = "DATA";
        const { data } = await (http2Faxios as unknown as TypedFaxiosInstance).post(localServerURL, payload);
    ...
    it("should enforce maxBodyLength for HTTP/2 streamed uploads", async () => {
      ...
      try {
        ...
        await assert.rejects(
          (http2Faxios as unknown as TypedFaxiosInstance).post(localServerURL, source, {
            maxBodyLength: 1024,
            headers: { "Content-Type": "application/octet-stream" },
          }),
          (error: any) => {
    ...
    it("should support FormData as a payload", async () => {
      ...
      try {
        ...
        const { data } = await (http2Faxios as unknown as TypedFaxiosInstance).post(localServerURL, form);
    ...
    describe("response types", () => {
      ...
      const fixtures = {
        ...
        arraybuffer: (value: any) =>
          assert.deepStrictEqual(value, Buffer.from(originalData)),
        stream: async (value: any) =>
          assert.deepStrictEqual(await getStream(value), originalData),
        json: async (value: any) =>
    ...
    it("should support request timeout", async () => {
      ...
      try {
        ...
        await assert.rejects(async () => {
          await (http2Faxios as unknown as TypedFaxiosInstance).get(localServerURL, {
            timeout: 500,
          });
        }, /timeout/);
    ...
    describe("session", () => {
      ...
      it(
        "should reuse session for the target authority",
        { retry: 2 },
        async () => {
          const server = await startHTTPServer(
            (_req: any, res: any) => {
          ...
          try {
            ...
            const [response1, response2] = await Promise.all([
              (http2Faxios as unknown as TypedFaxiosInstance).get(localServerURL, {
                responseType: "stream",
              }),
              (http2Faxios as unknown as TypedFaxiosInstance).get(localServerURL, {
                responseType: "stream",
      ...
      it(
        "should use different sessions for different authorities",
        { retry: 2 },
        async () => {
          const server = await startHTTPServer(
            (_req: any, res: any) => {
          ...
          try {
            ...
            const [response1, response2] = await Promise.all([
              ...
              (http2Faxios as unknown as TypedFaxiosInstance).get(localServerURL2, {
                responseType: "stream",
              }),
            ]) as any[];
      ...
      it(
        ...
        async () => {
          ...
          try {
            ...
            const response1 = await (http2Faxios as unknown as TypedFaxiosInstance).get(localServerURL, {
              responseType: "stream",
              http2Options: {
                sessionTimeout: 1000,
              },
            ...
            const response2 = await (http2Faxios as unknown as TypedFaxiosInstance).get(localServerURL, {
              responseType: "stream",
              http2Options: {
                sessionTimeout: 1000,
              },
  ...
  it("should reject when only the request socket emits an error", async () => {
    ...
    const error = await Promise.race([
      (axios as unknown as TypedFaxiosStatic).post("http://example.com/", "test", {
        transport,
        maxRedirects: 0,
      }),
  ...
  describe("keep-alive", () => {
      ...
      try {
        ...
        const results = await Promise.all(
          Array.from({ length: CONCURRENCY }, (_, i) =>
            (axios as unknown as TypedFaxiosStatic).get(`/req-${i}`, { baseURL, httpAgent: agent }),
          ),
        );
    ...
    it("should not leak memory via retained request closures under a long burst of keep-alive requests (regression #10780)", async () => {
      ...
      try {
        ...
        for (let i = 0; i < 200; i += 1) {
          const response = await (axios as unknown as TypedFaxiosStatic).get("/", { baseURL, httpAgent: agent }) as any;
    ...
    it('should not fail with "socket hang up" when using timeouts', async () => {
      ...
      try {
        const baseURL = `http://localhost:${(server.address() as AddressInfo).port}`;
        await (axios as unknown as TypedFaxiosStatic).get("/1", { baseURL, timeout: 1000 });
        await (axios as unknown as TypedFaxiosStatic).get("/wait", { baseURL, timeout: 0 });
      } finally {
        await stopHTTPServer(server);
      }
    }, 15000);
    ...
    it("should install at most one socket error listener across reused keep-alive sockets", async () => {
      ...
      await (axios as unknown as TypedFaxiosStatic).get("http://example.com/first", {
        transport,
        maxRedirects: 0,
      });
      ...
      for (let i = 0; i < 20; i += 1) {
        await (axios as unknown as TypedFaxiosStatic).get(`http://example.com/next-${i}`, {
          transport,
          maxRedirects: 0,
        });
    ...
    it("should not accumulate socket error listeners when a pooled socket is reassigned before the previous request closes (regression #10780)", async () => {
      ...
      const results = await Promise.all(
        Array.from({ length: 20 }, (_, i) =>
          (axios as unknown as TypedFaxiosStatic).get(`http://example.com/concurrent-${i}`, {
            transport,
            maxRedirects: 0,
          }),
        ),
    ...
    it("should route a socket error to the currently-active request after the socket has been reassigned", async () => {
      ...
      const err = await (axios as unknown as TypedFaxiosStatic)
  ...
  describe("redirect listener accumulation", () => {
    it("should not emit MaxListenersExceededWarning when a single request follows >= 11 redirects", async () => {
      ...
      try {
        const baseURL = `http://localhost:${(server.address() as AddressInfo).port}`;
        const response = await (axios as unknown as TypedFaxiosStatic).get("/start", {
          baseURL,
    ...
    it("should attach at most one close listener to the outer request across a long redirect chain", async () => {
      ...
      try {
        ...
        function record(this: any, eventName: string) {
          if (eventName === "close") {
  ...
  describe("socketPath security", () => {
    ...
    async function startUnixServer(socketPath: string) {
    ...
    it("allows socketPath when no allowedSocketPaths is set (backwards compatible)", async () => {
      ...
      try {
        const res = await (axios as unknown as TypedFaxiosStatic).get("http://localhost/echo", { socketPath }) as any;
    ...
    it("allows socketPath when it matches an allowedSocketPaths string", async () => {
      ...
      const server = await startUnixServer(socketPath);
      try {
        const res = await (axios as unknown as TypedFaxiosStatic).get("http://localhost/echo", {
          socketPath,
    ...
    it("rejects socketPath not in allowedSocketPaths", async () => {
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).get("http://localhost/echo", {
    ...
    it("rejects non-string socketPath", async () => {
      await assert.rejects(
        (axios as unknown as TypedFaxiosStatic).get("http://localhost/echo", { socketPath: 12345 as unknown as string }),
        (err: any) => {
          assert.ok(err instanceof FaxiosError);
          assert.strictEqual(err.code, FaxiosError.ERR_BAD_OPTION_VALUE);
