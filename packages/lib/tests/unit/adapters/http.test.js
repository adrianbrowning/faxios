...
describe("supports http with nodejs", () => {
  ...
  function toleranceRange(positive, negative) {
    ...
    return (actualValue, value) => actualValue > value ? actualValue <= value * p : actualValue >= value * n;
  }
  ...
  it("should support sockets", async () => {
    let socketName = path.join(
      os.tmpdir(),
      `axios-test-${process.pid}-${Date.now()}-${Math.random().toString(16)
        .slice(2)}.sock`
    );
  ...
  describe("socketPath security", () => {
    function makeSocketPath() {
      const pipe = `axios-socketpath-${process.pid}-${Date.now()}-${Math.random().toString(36)
        .slice(2)}`;
